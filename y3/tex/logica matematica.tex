...
\setcoursebook{integrato con le \underline{\href{https://sites.google.com/uniroma1.it/logicamatematica2526/dispense?authuser=0}{dispense}} del professor Carlucci}
\setauthorname{aglaia norza}
\setauthoremail{thisisaglaia@gmail.com}
\setauthorgithub{AglaiaNorza}
\setcoverphoto{stoplogic.jpg}
\setcoverwidth{0.7\textwidth}
\setcoverheight{0.7\textwidth}
...
(Notiamo che non è detto che l'essere numerabile implichi l'essere computabilmente numerabile)
...
\subsection{Conclusione del Teorema di Completezza (Proposizionale)}
...
\begin{itemize}
    ...
    \item \textbf{Conseguenza logica di una teoria} - \( E \) \ t.c. \(E \) è vero in tutti i modelli di \( T \) (diciamo che \( T \) \textit{implica logicamente} \( E \))
        \subitem definiamo coerentemente \( Conseq(T) = \{E : T\vDash E\} \) (si usa anche la notazione \( Cons(T) \))
\end{itemize}
...
\begin{defframe}{Sottostruttura elementare}{}
    Date due strutture \( \Aa \) e \( \Bb \) tali che \( \Aa \subseteq \Bb \), si dice che \( \Aa \) è \textbf{sottostruttura elementare} di \( \Bb \) (\( \Aa \preceq \Bb \)) se la verità delle formule è preservata prendendo parametri da \( A \).
...
\subsection{Proprietà del testimone tra \( \Rr \) e \(\Qq\)}
...
\begin{proofframe}{}{}
    ...
    \begin{itemize}
        ...
        Supponiamo per Ipotesi Induttiva che il teorema valga per le sottoformule \( A \) e \( B \). Ovvero:
                \[
        \Qq \vDash A[\alpha] \iff \Rr \vDash A[\alpha] \quad \text{e} \quad \Qq \vDash B[\alpha] \iff \Rr \vDash B[\alpha]
        \]
        ...
        \item \textbf{passo induttivo (esistenziale)}:
            ...
            \textit{Ipotesi Induttiva}: Assumiamo che per la formula interna \( G(x) \) e per qualsiasi assegnamento in \( \Qq \), l'equivalenza valga (\( \forall \beta \text{ in } \Qq, \Qq \vDash G[\beta] \iff \Rr \vDash G[\beta] \))

        Dobbiamo mostrare:
        \[
        \Qq \vDash \exists x \, G(x)[\alpha] \iff \Rr \vDash \exists x \, G(x)[\alpha]
        \]
        ...
        Poiché \( \Qq \subseteq \Rr \), questo \( q \) esiste anche in \( \R \).
        Applicando l'I.I. alla formula \( G(x) \) valutata su \( q \):
        \[
            \Qq \vDash G(x)[\alpha \ass{x}{q}] \implies \Rr \vDash G(x)[\alpha \ass{x}{q}]
        \]
        Dato che abbiamo trovato un elemento in \( \R \) (lo stesso \( q \)) che soddisfa la formula, concludiamo:
        \[
            \Rr \vDash \exists x \, G(x)[\alpha]
        \]

        \textit{Direzione \(\impliedby\):}
        Assumiamo \(\Rr \vDash \exists x \, G(x)[\alpha]\).

        Normalmente, il testimone in \( \R \) potrebbe essere irrazionale. Ma la \textit{Proprietà del Testimone} garantisce che se esiste una soluzione in \( \R \), allora esiste un testimone \(q \in \Q\) tale che:
        \[
            \Rr \vDash G(x)[\alpha \ass{x}{q}]
        \]
        Ora che abbiamo un parametro \( q \) razionale, possiamo usare l'I.I.:
        \[
            \Rr \vDash G(x)[\alpha\ass{x}{q}] \iff \Qq \vDash G(x)[\alpha\ass{x}{q}]
        \]
        Quindi \(\Qq \vDash G(x)[\alpha\ass{x}{q}]\). Poiché abbiamo trovato un testimone in \( \Qq \), per definizione di soddisfacibilità:
        \[
            \Qq \vDash \exists x \, G(x)[\alpha]
        \]
...
Abbiamo quindi ottenuto che, se vale la Proprietà del Testimone tra \( \Rr \) e \( \Qq \), si ha \( \Qq \prec \Rr \).
...
\subsubsection{Costruzione di \( \Aa \)}
...
\subsection{Generalizzazione della dimostrazione}
    ...
    La Proprietà del Testimone si esprime non relativamente a \( \Qq \) ma ad una sottostruttura numerabile \( \Xx \) di \( \Bb \) del tipo \( (X, \leq_\Xx ) \) con \( X \subseteq B \) e \( \leq_\Xx = \leq_\Bb \cap (X \times X)\).

    Si parte da un sottoinsieme numerabile \( X \) di \( \Bb \) (che esiste sicuramente), e si procede per chiusura sotto funzioni di Skolem allo stesso modo.

    Si costruisce così \( \Xx = \bigcup_{i\in \N} X_i \).\\ 
    Si ha \( \Xx \vDash  \)DLO, e quindi \( \iso  \) e \( \equiv \Qq\).
...
Poiché DLO è completa e \( \Qq \) ne è un modello, e poiché DLO e \( \Rr \) condividono la stessa teoria nel linguaggio degli ordini, vale:
\[ \text{DLO} \vDash E \iff \Qq \vDash E \iff \Rr \vDash E \]

Questo significa che (limitatamente al linguaggio degli ordini)
\begin{itemize}
    \item se un enunciato \( E \) è vero in \( \Rr \), DLO \(\vDash E  \)
    \item se un enunciato \( E \) è falso in \( \Qq \), DLO \( \vDash \neg E \)
\end{itemize}
...
Può essere utile anche ricordare che:
\begin{itemize}
    \item DLO è una teoria assiomatizzabile in modo finito (ha 7 assiomi) ed è completa. Ogni teoria computazionalmente numerabile e completa è \textbf{decidibile}. Esiste quindi un algoritmo per stabilire in tempo finito se un enunciato \(E\) appartiene a \(Cons(\text{DLO})\).
\end{itemize}

\section{Criterio di Tarski-Vaught}
\label{criterio-tarski}
...
\begin{proofframe}[title=idea di dimostrazione]
...
\begin{gframe}{}
    \begin{enumerate}
        ...
        \item \( \forall x F\to F [t/x] \) con \( t \) termine libero per \( x \) in \( F \) {\small \color{gray} (per ``libero'' vedi \underline{p.\pageref{terlib}})}
            \subitem da questa regola deriva anche:
            \AxiomC{\( F[t/x] \)}
            \UnaryInfC{\( \exists x F{\color{gray}(x)}\)}
            \DisplayProof
...
\section{Teorema di Completezza per la Logica Predicativa}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \( T \in \texttt{SAT} \): se \( T\in \SAT \) e \( T\vDash A \), sappiamo che \( T \cup \neg A \in \UNSAT \). 
            ...
            per reductio ad absurdum {\small(se un'ipotesi porta a un assurdo, allora l'ipotesi deve essere falsa, ovvero \((\neg A \to \bot) \vdash A \))}, si ha quindi \( T \vdash A \).}
    \end{itemize}
...
\subsection{Modello dei Termini}
...
\begin{defframe}{Modello dei termini}{}
...
\begin{itemize}
    ...
    \item l'interpretazione di un simbolo di relazione \( \Rr \) è data dall'insieme dei termini chiusi di cui \( S \) dimostra che soddisfano la relazione - ovvero \( (t_1, \dots, t_n) \in \Rr^\Mm_i \iff  S \vdash \Rr_i(t_1, \dots, t_n)\)
    \item l'interpretazione di un simbolo di funzione \( f \) è l'associazione \( t_1, \dots, t_n \mapsto f(t_1, \dots, t_n) \)\\ in cui \( f^\Mm_j : M^n \to M \) è t.c. \( f^\Mm_j (t_1, \dots, t_n) := f_j(t_1, \dots, t_n) \)
...
\begin{proofframe}{}
    Procediamo per induzione sulla complessità della formula \( E \).\\
    Assumiamo che per ogni formula \( A \) con complessità minore di \( E \), valga \( \Mm \vDash A \iff S \vdash A \)
    \begin{itemize}
        \item \( E \)  è \( \neg G \)

            \subitem se \(\Mm \vDash \neg G\) allora \( \Mm\not\vDash G \) e per I.I. \( S\not\vdash G \) - per completezza, \( S\vdash \neg G \)

            \subitem se \(\Mm \not\vDash \neg G\) allora \( \Mm\vDash G \) e per I.I. \( S\vdash G \) - per coerenza, \( S\not\vdash \neg G \)
        ...
        \item \( \Mm \vDash G \land H \) {\color{CadetBlue}\( \iff S\vdash G \land H \)}
            ...
            \subitem per ipotesi induttiva, segue \( \Mm\vDash G \ \land \ \Mm \vDash H \), da cui \( \Mm\vDash G \land H \)
        ...
        \item se \( E \) è del tipo \( \forall x F \) {\color{gray}\small(vogliamo \( \Mm \vDash \forall x F \iff S \vdash \forall x F \) )}
            
            sappiamo \( \Mm \vDash \forall x F \iff \forall m \in M, \ \Mm \vDash F \tworowmatrix{x}{m} \)\\
        {\color{CadetBlue}e qui, poiché l'interpretazione di un termine corrisponde al termine stesso, vale\\ \( \Mm \vDash F \tworowmatrix{x}{m} \iff \Mm \vDash F[x/m]\)}
        \subitem per ipotesi induttiva, l'equivalenza vale sulle istanze: \( \Mm \vDash F[x/m] \iff S\vdash F[x/m] \).

        \subitem \( (\impliedby) \) è immediato: se \( S \vdash \forall x F \), allora per le regole logiche \( \forall m, S \vdash F[x/m] \). Per l'I.I. allora \( \forall m, \Mm \vDash F[x/m] \), e quindi \( \Mm \vDash \forall x F \).

        \subitem \( (\implies) \) sorge il problema - sappiamo che \( \forall m, S \vdash F[x/m] \) (ogni istanza è dimostrabile), ma questo non implica automaticamente \( S \vdash \forall x F \).
        
        {\color{gray}\small (è come dire che sappiamo che per ogni numero c'è una dimostrazione, ma vogliamo che ci sia un'unica dimostrazione che valga per tutti i numeri)}
    \end{itemize}
\end{proofframe}

\subsection{Teorie con Testimoni}
Possiamo definire una teoria che ci aiuti nella dimostrazione.
...
\begin{gframe}{Testimoni Sintattici vs Testimoni Semantici}
    Le \textit{Teorie con Testimoni} di Henkin e la \textit{Proprietà del Testimone} di Tarski-Vaught (\underline{p.\pageref{criterio-tarski}}) sono `due facce della stessa medaglia':
    \begin{itemize}
        \item \textbf{Tarski-Vaught (Semantica):} riguarda i modelli (\( \mathcal{A} \subseteq \mathcal{B} \)) - garantisce che se il modello grande soddisfa un enunciato esistenziale, il testimone concreto si trova già nel dominio del modello piccolo.
        \item \textbf{Henkin (Sintassi):} riguarda le teorie e il linguaggio (\( T \vdash \dots \)) - garantisce che se la teoria dimostra un enunciato esistenziale, esiste un termine chiuso \( t \) nel linguaggio che fa da testimone.
    \end{itemize}
    Il "ponte" tra le due è la costruzione del \textit{modello dei termini}: poiché gli oggetti del dominio di questo modello \textit{sono} i termini stessi, il trucco di Henkin (aggiungere testimoni sintattici al linguaggio) è esattamente ciò che ci assicura che il nostro modello soddisferà la proprietà semantica dei testimoni.
\end{gframe}

Vediamo come una teoria con testimoni ci aiuta a dimostrare il caso \( \forall x F(x) \) con \( F(x) \) aperta.

\begin{proofframe}[title=Dimostriamo che se \( \Mm \vDash \forall x F \) allora \(S \vdash \forall x F\)]

Per assurdo, supponiamo \( \Mm \vDash \forall x F \) e \( S \not\vdash \forall x F \).

Per completezza di \( S \) vale \( S \vdash \neg E \), ovvero \( S \vdash \exists x \neg F(x) \). Dato che \( S \) è una teoria con testimoni, esiste un termine chiuso \( t \) tale che \( S \vdash \exists x \neg F(x) \to \neg F (t) \). 

Dunque \( S\vdash \neg F(t) \).

Per ipotesi induttiva, \( \Mm \vDash \neg F(t)\). Ma per ipotesi iniziale vale \( \Mm \vDash \forall x F(x) \), ovvero vale per qualsiasi elemento del dominio (che qui sono i termini). Quindi, vale in particolare per \( t \) - ovvero \( \Mm \vDash F(t) \).

Dunque, otteniamo \( \Mm \vDash F(t)\) e \( \Mm \vDash \neg F(t) \) - contraddizione. \qed
...
\begin{thmframe}{Estensioni con testimoni di Teorie}{}
...
\begin{proofframe}{}{}
    Sia \( T \) una teoria coerente nel linguaggio \( \Ll \).

    Sia \( B = \{c_1, c_2, c_3, \dots\} \) un insieme infinito numerabile di nuovi simboli di costante.
    Sia \( \Ll' = \Ll \cup B \) il linguaggio esteso.

    Sia \( T_0  = T \ \cup \ \{\text{istanze degli assiomi logici nel linguaggio esteso}\}\).

    Fissiamo un'enumerazione di tutte le formule di \( \Ll' \) che hanno esattamente una variabile libera: 
    \[ F_1(x_1), F_2(x_2), \dots, F_n(x_n), \dots \]
Costruiamo induttivamente una successione di costanti \( b_{j_1}, b_{j_2}, \dots \in B \).

    Per ogni \( n \ge 1 \), sia \( b_{j_n} \) la prima costante dell'insieme \( B \) (secondo un ordine fissato) tale che:
    \begin{itemize}
        \item \( b_{j_n} \) non compare nelle formule \( F_1(x_1), \dots, F_n(x_n) \);
        \item \( b_{j_n} \) è distinta da tutte le costanti precedentemente scelte \( b_{j_1}, \dots, b_{j_{n-1}} \).
    \end{itemize}
    
    {\small \color{gray}(garantiamo che \( b_{j_n} \) sia una costante "nuova" per la formula \( F_n \))}\\
    ...
    Sia \( W_n \) il seguente enunciato (detto "assioma di Henkin" per \( F_n \)):
    \[ W_n: \quad \exists x_n \neg F_n(x_n) \to \neg F_n(b_{j_n}) \]

    Sia \( T_n = T_0 \cup \{W_1, \dots, W_n\} \), e sia \( T_{\infty} = \bigcup_n T_n \).

    \textbf{claim}: \( T_{\infty} = \bigcup_{n\in \N} T_n \) è di Henkin, coerente ed estende \( T \).
...
\subsection{Conclusione del Teorema di Completezza (Predicativo)}
Possiamo quindi formalizzare i seguenti teoremi:

\begin{thmframe}{Modello di una Teoria con Testimoni}{}
    Sia \( T \) una teoria con testimoni e coerente in un linguaggio numerabile. Allora, \( T \) ha un modello numerabile.
\end{thmframe}

\begin{thmframe}{Teorema di Completezza (esistenza del modello)}{}
    Ogni teoria coerente (in un linguaggio numerabile) ha un modello numerabile.
\end{thmframe}

\begin{proofframe}[title=passi per dimostrare la Completezza]
...
\begin{pframe}
   Il Teorema di Compattezza può essere visto come una conseguenza diretta (corollario) del \textbf{Teorema di Completezza} appena dimostrato.

   Infatti:
   \begin{itemize}
        \item una dimostrazione formale è, per definizione, una sequenza finita di formule
        \item quindi, se una teoria è incoerente (cioè dimostra una contraddizione, \( T\vdash\bot \)), deve esistere un sottoinsieme finito di assiomi da cui deriva la contraddizione
        \item di conseguenza, se ogni sottoinsieme finito è coerente, allora l'intera teoria è coerente
        \item per il Teorema di Completezza, se una teoria è coerente, allora ha un modello.
    \end{itemize}
\end{pframe}
...
Ha ancora più senso chiedersi, nello specifico, data una classe di strutture \( \Cc \) e un linguaggio predicativo \( \Ll \), se esista una teoria in \(\Ll \) tale che la proprietà sopra sia soddisfatta.
...
\subsubsection{Finitezza e infinitezza}
...
\begin{lemmaframe}{}{}
    ...
    \begin{gframe}{Nota / fun fact !}
    ``Equivalente'' al Teorema di Post della Teoria della Computazione (\( REC \cap coREC = DEC \)).
    
    (assiomatizzabile \(\iff\) semidecidibile ,
    \\finitamente assiomatizzabile \(\iff\) decidibile)
    \end{gframe}
\end{lemmaframe}
...
\begin{pframe}
    Sia $E$ l'enunciato (o la congiunzione finita di enunciati) che assiomatizza finitamente $P$.
    Quindi, per ogni struttura $\Aa$:
    \[ \Aa \in P \iff \Aa \vDash E \]
    Poiché per ipotesi $T$ assiomatizza $P$, ogni modello di $T$ deve avere la proprietà $P$, e quindi deve soddisfare $E$:
    \[ T \vDash E \]
    Per il \textbf{Teorema di Compattezza}, se $T\vDash E$, allora allora esiste un sottoinsieme finito $T_0 \subseteq T$ tale che $T_0 \vDash E$.
    ...
    \begin{itemize}
        ...
        \item[($\Rightarrow$)] Se $\Aa \vDash T_0$, dato che abbiamo stabilito $T_0 \vDash E$, allora $\Aa \vDash E$. Poiché $E$ assiomatizza $P$, allora $\Aa \in P$.
    \end{itemize}
...
\end{lemmaframe}
\begin{pframe}
    Usiamo il Teorema di Compattezza.
    \begin{enumerate}
        \item per ogni \( n \in \mathbb{N} \), sia \( \varphi_n \) la formula che esprime "esistono almeno \( n \) elementi distinti". Definiamo l'insieme \( \Gamma = T \cup \{ \varphi_n \mid n \in \mathbb{N} \} \).
        
        \item per il Teorema di Compattezza, \( \Gamma \) ha un modello se e solo se ogni suo sottoinsieme finito \( \Delta \subset \Gamma \) è soddisfacibile.
        
        \item sia \( \Delta \) un arbitrario sottoinsieme finito di \( \Gamma \). Sia \( N \) il massimo intero tale che \( \varphi_N \in \Delta \). Per ipotesi, \( T \) ha modelli arbitrariamente grandi, quindi esiste un modello \( \mathcal{M} \) di \( T \) con almeno \( N \) elementi. Questo modello soddisfa \( T \) e tutte le \( \varphi_n \) in \( \Delta \), rendendo \( \Delta \) soddisfacibile.
        
        \item poiché ogni sottoinsieme finito è coerente, \( \Gamma \) ammette un modello \( \mathcal{A} \). Poiché \( \mathcal{A} \models \Gamma \), allora \( \mathcal{A} \models T \) e \( \mathcal{A} \) deve soddisfare \( \varphi_n \) per ogni \( n \), implicando che \( \mathcal{A} \) è infinito.
    \end{enumerate}
\end{pframe}

\begin{gframe}{(Non) assiomatizzazione di ``essere un grafo connesso''}
    Consideriamo i grafi semplici con linguaggio \( \mathcal{L} = \{E(x,y)\} \).
    Sia \( P = \) ``essere un grafo connesso''.

    Definiamo la proprietà \( D_n = \) ``\( c \) e \( d \) sono a distanza \( \geq n \)'' (estendendo il linguaggio con le costanti \( c, d \)):
    \[ 
    D_n \equiv \neg \left(\exists x_1, \dots, x_n \left(\bigwedge_{i \neq j}\neg (x_i = x_j) \land E(c, x_1)\land \dots \land E(x_n, d)\right)\right)
    \]
    \begin{itemize}
        \item Supponiamo che \( P \) sia assiomatizzabile da una teoria \( C \).
        \item Consideriamo la teoria \( C' = C \cup \{D_n \mid n \ge 0\} \) - \( C'\) non dovrebbe avere un modello (un grafo non può essere connesso se la distanza tra \( c \) e \( d \) è infinita).
        \item Prendiamo un sottoinsieme finito \( T_0 \subseteq_{fin} C' \).
            \subitem sia \( k = \max \{ n \mid D_n \in T_0 \} \).
    \subitem esiste un modello per \( T_0 \): basta prendere un grafo connesso con due vertici a distanza \( > k \)
\item abbiamo \( \FINSAT \to \SAT \)
\item esiste quindi un modello di \( C' \) che è connesso, ma dove \( c \) e \( d \) non sono connessi (per le \( D_n \)) - contraddizione \qed
    \end{itemize}
\end{gframe}
...
Un sottoinsieme qualsiasi \( T_0 \subseteq T \cup \{A_n \mid n\in \N\} \) ha un modello - basta dare un'interpretazione alla costante \( c \).
...
\begin{defframe}{Funzioni calcolabili}{}
    ...
    \begin{itemize}
        ...
        \item \( i(x, y) = \begin{cases}
                ...
                0 & \text{se } x \neq y
        \end{cases} \)
    \item \( \pi_i^n (x_1, \dots, x_n) = x_i \) \ {\color{CadetBlue}(proiezione)}
    \end{itemize}
...
\end{defframe}
\begin{gframe}{Nota sulle funzioni calcolabili}
  Normalmente, nella teoria della calcolabilità classica (secondo il modello di Kleene delle funzioni parziali ricorsive), la classe viene definita partendo da un insieme di funzioni base ancora più elementare: 
\begin{itemize}
    \item la funzione \textit{zero} $Z(x) = 0$
    \item la funzione \textit{successore} $S(x) = x + 1$
    \item le proiezioni $\pi_i^n$
\end{itemize}
In quel contesto, oltre alla composizione e alla minimalizzazione, viene introdotta un'ulteriore operazione di chiusura: la ricorsione primitiva. 

La definizione adottata in questa sede è del tutto equivalente, ma adotta una "scorciatoia": omette la ricorsione primitiva e la compensa promuovendo addizione ($+$), moltiplicazione ($*$) e test di uguaglianza a funzioni base (approccio utile per snellire la trattazione ed evitare di dover ricostruire l'aritmetica elementare da zero per le dimostrazioni successive ).
\end{gframe}


\begin{propframe}{Tesi di Church-Turing}{}
    Una funzione è intuitivamente \textit{calcolabile} se e solo se appartiene alla classe $\mathcal{C}$.

    {\color{gray}(``Se un problema è umanamente calcolabile, allora esisterà una macchina di Turing in grado di risolverlo (cioè di calcolarlo)'')}
\end{propframe}
...
\begin{defframe}{Definibilità}{}
    Diciamo che \( \varphi \) è \textbf{rappresentabile} (o definibile) in \( \Nn \) da una formula \( F(x_1, \dots, x_k, y) \) se per ogni tupla di numeri naturali \((a_1, \dots, a_k, b) \subseteq \mathbb{N}^{k+1}\), vale l'equivalenza:
    \[
...
\begin{thmframe}{Teorema di Definibilità}{}
    Le funzioni calcolabili sono definibili (o rappresentabili) in \( \Nn \).
\end{thmframe}
...
{\color{gray}quindi non esiste una formula \( V \) t.c.  \( E \in Th(\Nn) \iff \Nn \vDash V(\overline{code(E)})\)}
\end{thmframe}
...
Introduciamo una nuova funzione \( \delta : \N \to \N \) (``\textbf{funzione diagonale}'').\\
Dato \( p\in \N \), \( \delta \) controlla se \( p \) è il codice numerico di una formula \( A(x) \) una variabile libera. Se lo è, \( \delta(p) =  \) codice di \( A(\bar{p}/x) \).
...
\( \delta \) è evidentemente calcolabile (uso la tesi di Church-Turing per non doverlo dimostrare esplicitamente).\\ In quanto calcolabile, \( \delta\in\Cc \), e quindi \( \delta \) rappresentabile.

Quindi, sia \( D(x, y) \) una formula che rappresenta \( \delta \) in \( \Nn \), ovvero tale che:
\[ \delta(a) = b \iff \Nn \vDash D(\bar{a}, \bar{b})\]

Consideriamo la formula
\[ A(x):= \forall y (D(x, y)\to \neg V(y)) \]

\begin{gframe}{}
la formula dichiara ``data una formula \( x \), la sua diagonalizzazione (applicazione del suo stesso codice a se stessa) è falsa in \( \Nn \)''

{\color{gray}\small
più precisamente: per l'unico \( y \) che è il codice della formula ottenuta sostituendo il numerale \(\bar{x}\) dentro la formula con codice \( x \), vale che \( y \) non rappresenta un enunciato vero (cioè non soddisfa \( V \)).}
\end{gframe}
Questa formula è una formula con una variabile libera \( x \), e in quanto tale può essere codificata.

Sia \( p \) il codice di \( A(x) \).

Consideriamo ora l'enunciato \( A(\bar{p}) \), ottenuto sostituendo a \( x \) il numerale del suo stesso codice:
    \[ A(\bar{p}) := \forall y (D(\bar{p}, y)\to \neg V(y)) \]
    \begin{itemize}
        \item la funzione diagonale, applicata al codice \( p \) (che rappresenta \( A(x) \)), calcola il codice della formula \( A(\bar{p}) \).
        \item quindi, l'unico numero \( y \) che soddisfa \( D(\bar{p}, y) \) è proprio \( q = \text{code}(A(\bar{p})) \).
    \end{itemize}

    Pertanto, la formula \( A(\bar{p}) \) sta parlando di se stessa - essa afferma:
    \begin{center}
        \textit{``L'enunciato con codice \( q \) (cioè io stessa) non soddisfa \( V \)''}
    \end{center}

    \begin{gframe}{Il Paradosso del Mentitore}
        Se assumiamo che \( V(y) \) rappresenti la verità (cioè \( V(y) \) è vero se e solo se l'enunciato con codice \( y \) è vero), allora \( A(\bar{p}) \) è l'equivalente formale del paradosso del mentitore:
        \[ \text{``Io sono falsa''} \]
    \end{gframe}

Dimostriamolo formalmente, ovvero che l'ipotesi che la verità sia esprimibile porti alla contraddizione\\ \( \Nn \vDash V(q) \land \Nn \vDash \neg V(q) \).

\begin{proofframe}[title=dimostrazione breve]
    Sia \( p \) il codice della formula \( A(x) \).
    Sia \( q \) il codice della formula ottenuta per diagonalizzazione \( A(\bar{p}) \).

    Per le proprietà della funzione diagonale rappresentata da \( D \), sappiamo che:
    \[ \Nn \vDash \forall y (D(\bar{p}, y) \longleftrightarrow y = \bar{q}) \]

    Quindi, valutiamo la verità di \( A(\bar{p}) \) in \( \Nn \):
    \begin{align*}
        \Nn \vDash A(\bar{p}) & \iff \Nn \vDash \forall y (D(\bar{p}, y) \to \neg V(y)) \\
                              & \iff \Nn \vDash \forall y(y = q \to \neg V(\bar{q})) \quad \text{(sse di sopra)}\\
                              & \iff \Nn \vDash \neg V(\bar{q})     
    \end{align*}

    D'altra parte, assumiamo per assurdo che \( V(x) \) rappresenti la verità aritmetica \( Th(\Nn) \). Per definizione di rappresentabilità deve valere:
    \[ \Nn \vDash A(\bar{p}) \iff \Nn \vDash V(\overline{code(A(\bar{p}))}) \iff \Nn \vDash V(\bar{q}) \]

    Unendo le due equivalenze otteniamo la contraddizione:
    \[ \Nn \vDash V(\bar{q}) \iff \Nn \vDash \neg V(\bar{q}) \]

    Pertanto, l'assunzione iniziale è falsa: \( Th(\Nn) \) non è rappresentabile in \( \Nn \).
\end{proofframe}

\begin{proofframe}[title=dim fatta in classe]
Dato che la formula \( D\) definisce la funzione \( \delta \) in \( \N \), abbiamo che
\[ \Nn \vDash D(p, q) \]

Dimostriamo \( \Nn \vDash \neg V(q) \):
\begin{itemize}
    \item \textbf{caso 1}: \( \Nn \not\vDash A(p) \) 
        \subitem \( A(p) \) non è una verità in \( \Nn \), dunque \( code(A(p)) = q \not\in Th(\Nn) \), dunque per ipotesi \( \N \vDash \neg V(q) \)
    \item \textbf{caso 2}: \( \N \vDash A(p) \) 
        \subitem se \( A(p) \) è vero, è vero per \( y = q \) \ {\color{gray}\small(\( \Nn \vDash D(p, q)\to \neg V(q) \))},\ \  quindi, dato che \( \N \vDash D(p, q) \) otteniamo \( \Nn \vDash \neg V(q) \)
\end{itemize}


Dimostriamo ora \( \Nn \vDash V(q) \):

Da \( \Nn\vDash D(p,q) \) e dalla funzionalità della formula \( D \) segue \( \Nn \vDash (p, y ) \to y = q \).

Visto che abbiamo appena dimostrato \( \Nn \vDash \neg V(q) \), abbiamo {\color{gray}\small(se non vale per \( q \), sicuramente se \( y=q \) non vale per \( y \))}
\[ \Nn \vDash y = q \to \neg V(y) \]
Dunque, {\color{gray}\small(poiché \( \Nn \vDash (p, y ) \to y = q \))}, abbiamo anche
\[ \Nn \vDash D(p, y)\to \neg V(y) \]

Se vale con variabile libera allora vale anche nella forma
\[ \Nn \vDash \forall y (D(p, y)\to \neg V(y)) \]
che è esattamente \( A(p) \).

Dunque, \( A(p)\in Th(\Nn) \), ovvero \( A(p) \) è una verità aritmetica.

Quindi, dato che \( V \) rappresenta \( Th(\Nn) \), abbiamo
\[ \Nn \vDash V(code(A(p))) = V(q) \]
il che è impossibile.
\end{proofframe}

Abbiamo quindi:
\begin{corframe}{}{}
    L'insieme delle verità aritmetiche \( Th(\Nn) \) non è algoritmicamente decidibile.
\end{corframe}

\begin{itemize}
    \item \( Th(\Nn) \) non è neanche semidecidibile (/computazionalmente enumerabile) - se lo fosse, poiché \( \Nn \) è un modello completo, sarebbe anche decidibile
\end{itemize}
...
\begin{thmframe}{Primo Teorema di Incompletezza (forma debole)}{}
    Se \( T \) è una teoria decidibile e tale che \( \Nn \vDash T \), allora \( T \) non è completa: esiste un enunciato \( G \) tale che \( T\not\vDash G \land T \not\vdash\neg G \)
\end{thmframe}
\begin{pframe}
    Se \( T \) fosse completa, dato che \( \N\vDash T \) seguirebbe che \( \{E \mid T\vdash E\} \subseteq \{E \mid \Nn \vDash E\} \). In più, per completezza, poiché abbiamo \( T\vdash E \xor T\vdash \neg E \), e poiché, visto che \( \N \vDash T \), \( E \) è o vero o falso in \( \N \), abbiamo
    \[  \{E \mid T\vdash E\} = \{E \mid \N \vDash E\} =Th(\Nn)  \]
...
\begin{mdframed}
    \begin{itemize}
        \item[(Ax 1)] \( 0 + 1 = 1 \quad \text{definizione dell'unità (1)} \)
        \item[(Ax 2)] \( \forall x(x + 1 \neq 0) \quad \text{lo zero non è un successore} \)
        \item[(Ax 3)] \( \forall x(x \neq 0 \to \exists z(z + 1 = x)) \quad \text{esistenza del predecessore} \)
        \item[(Ax 4)] \( \forall x\forall y(x + 1 = y + 1 \to x = y) \quad \text{iniettività del successore} \)
        \item[(Ax 5)] \( \forall x(x + 0 = x) \quad \text{elemento neutro dell'addizione} \)
        \item[(Ax 6)] \( \forall x\forall y(x + (y + 1) = (x + y) + 1) \quad \text{definizione ricorsiva dell'addizione} \)
        \item[(Ax 7)] \( \forall x(x \times 0 = 0) \quad \text{annullamento della moltiplicazione} \)
        \item[(Ax 8)] \( \forall x\forall y(x \times (y + 1) = (x \times y) + x) \quad \text{definizione ricorsiva della moltiplicazione} \)
        \item[(Ax 9)] \( \forall x\forall y(x < y \lor x = y \lor y < x) \quad \text{legge di tricotomia dell'ordine} \)
    \end{itemize}
\end{mdframed}
...
Consideriamo l'enunciato \( A(x)\equiv \forall y \neg F(x, y) \) (che dice ``qualunque sia la formula associata ad x, non esiste alcuna dimostrazione dell'enunciato che si ottiene prendendo quella formula e dandole in pasto x stesso'') 
...
\subsection*{Vademecum per esercizi su MA (Aritmetica Minimale)}
Riassumiamo le informazioni fondamentali sull'Aritmetica Minimale (MA) e sul suo rapporto con il modello standard dei numeri naturali \( \mathcal{N} \).

A differenza di DLO, MA è una teoria incompleta (manca del Principio di Induzione). Questo crea una netta separazione tra ciò che è vero in \( \mathcal{N} \) e ciò che è dimostrabile da MA. 

È utile ricordare che:
\begin{itemize}
    \item tutto ciò che MA dimostra è vero in \( \mathcal{N} \)
        \subitem \( \text{MA} \vdash E \implies \mathcal{N} \models E \)
        \subitem \( \text{MA} \vdash \neg E \implies \mathcal{N} \models \neg E \) (ovvero \( \mathcal{N} \not\models E \))
    
    \item ma non vale l'inverso - MA è incompleta (quindi moltissimi enunciati veri in \( \Nn \) non sono dimostrabili da MA
        )       
        \subitem \( \mathcal{N} \models E \not\implies \text{MA} \vdash E \)

\item se un enunciato \( E \) è vero in \( \mathcal{N} \) ma non è dimostrabile in MA (\( \text{MA} \not\vdash E \)), la sua negazione è coerente con MA (anche se falsa in \( \Nn \))
\begin{itemize}
    \item sia \( E \) la proprietà commutativa: \( \mathcal{N} \models E \) ma \( \text{MA} \not\vdash E \).
    \item di conseguenza, la teoria \( \text{MA} \cup \{\neg E\} \) è coerente e ha modelli.
    \item questi modelli soddisfano gli assiomi di MA ma l'addizione non commuta (modelli non-standard).
    \item una ``estensione di MA'', anche se coerente, non è quindi necessariamente vera in \( \Nn \)
\end{itemize}
...
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}X|c|>{\raggedright\arraybackslash}p{7cm}|}
\hline
\textit{Premessa / Affermazione} & \textit{V/F} & \textit{Motivo} \\ \hline
Se $MA \vdash E$ allora $\mathcal{N} \models E$ & \textbf{V} & $\mathcal{N}$ soddisfa tutto ciò che MA dimostra. \\ \hline
Se $\mathcal{N} \models E$ allora $MA \vdash E$ & \textbf{F} & Incompletezza \\ \hline
Se $\mathcal{N} \models E$, $MA \cup \{\neg E\}$ non ha modelli & \textbf{F} & Equivale a dire $MA \vdash E$, falso per incompletezza. \\ \hline
Se $\mathcal{N} \not\models E$ allora $MA \vdash \neg E$ & \textbf{F} & Speculare all'incompletezza. \\ \hline
Esiste $E$ t.c. $MA \vdash \neg E \to (\overline{0} = \overline{1})$ & \textbf{V} & Basta scegliere $E$ come una tautologia  \\ \hline
Se $T$ estensione decidibilmente assiomatizzabile e coerente di MA, l'insieme degli enunciati dimostrabili in $T$ è computabilmente enumerabile & \textbf{V} & Segue da \( T \) decidibile \\ \hline
Se $T$ estensione decidibilmente assiomatizzabile e coerente di MA, l'insieme degli enunciati NON dimostrabili in $T$ è computabilmente enumerabile & \textbf{F} & Se lo fossero, poiché i teoremi sono già c.e., la teoria sarebbe decidibile. Ma per Incompletezza, ogni estensione coerente e assiomatizzabile di MA è indecidibile. \\ \hline
\end{tabularx}
\end{table}
