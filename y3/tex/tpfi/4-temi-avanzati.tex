...
Dato un insieme parzialmente ordinato (\textit{poset}, partially-ordered set) $(D, \apprord)$ formato da un dominio $D$ e una relazione d'ordine $\apprord$ su elementi di $D$ (non necessariamente tutti), definiamo una \textbf{catena} $C$ come un sottoinsieme di $D$ totalmente ordinato (in cui tutti gli elementi sono in relazione).

Un poset $(D, \apprord)$ si dice un Complete Partial Order se tutte le catene hanno un supremum, ovvero un elemento "limite" $\sqcup x = \lim_{n\to\infty} x_n$ che soddisfa le seguenti condizioni:
\begin{itemize}
...
$
\begin{itemize}
    \item non è computabile perché non è continua: $\sqcup f \ x_n = \bot \neq 1 = f(\sqcup \ x_n)$
\end{itemize}
\end{itemize}
