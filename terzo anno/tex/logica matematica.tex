...
\begin{gframe}{Colorabilità}
    \vspace{-1.5em}
\subsection*{Esempio: 2-colorabilità}
...
\begin{pframe}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Se \(\alpha(C)=1\), allora \(\alpha(B)=0\). Per ipotesi induttiva \( \Gamma \vdash C \). Dobbiamo dimostrare \( \Gamma \vdash \neg B \) (cioè \( \neg\neg C \)). Usiamo il teorema \( \vdash C \to \neg\neg C \).
        \end{itemize}
...
\begin{pframe}
    ...
    Consideriamo due assegnamenti \( \alpha \) e \( \beta \) che differiscono solo per il valore di \( p_k \):
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item Questo equivale a dire che \( \vDash (A_1 \to (\dots \to A)\dots) \) è una tautologia.
...
\begin{gframe}{}
    ...
    Ovvero \( Th(\Aa) \), l'insieme di tutte le frasi vere in una specifica struttura \(\Aa\), è una teoria completa.
\end{gframe}
\end{defframe}
...
\begin{itemize}
    \item monotonia - \( T\vDash \varphi \implies S \vDash \varphi \)

    \item "ereditarietà" dei modelli - ogni modello di \( S \) è anche modello di \( T \) (tutto \( T \) si trova dentro \( S \))
        \subitem \( Mod(S) \subseteq Mod(T) \)

    \item coerenza - se \( S \) è corente, allora anche \( T \) lo è
...
\begin{itemize}
    ...
    \item \textbf{caso 2}: \( \N \vDash A(p) \) 
       \subitem se \( A(p) \) è vero, è vero per \( y = q \)
