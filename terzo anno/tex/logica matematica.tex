...
\begin{proofframe}[title=dim]
    ...
    \textit{Base dell'induzione (\(i=1\)):} \( F_1 \) è l'inizio della derivazione. Ci sono tre casi:
    \begin{itemize}
        ...
        \item \textbf{Caso 2:} \( F_1 \) è proprio la formula \( B \). \\
        Dobbiamo dimostrare \( T \vdash B \to B \).
        Sappiamo che \( \vdash B \to B \) è un teorema logico (già dimostrato precedentemente o verificabile dagli assiomi).
    ...
    \begin{itemize}
        ...
        Per \textbf{ipotesi induttiva} sappiamo che:
        \begin{align*}
            1) \ & T \vdash B \to F_m \\
...
\begin{lemmaframe}{Lemma principale}{}
...
\begin{enumerate}
    \item Dal Teorema di Compattezza: \( T \vDash A \implies \exists \{A_1, \dots, A_n\} \subseteq T \) tale che \( A_1, \dots, A_n \vDash A \).
    ...
    \item Poiché \( \{A_1, \dots, A_n\} \subseteq T \), per la proprietà di monotonia concludiamo:
    \[ T \vdash A \]
\end{enumerate}
