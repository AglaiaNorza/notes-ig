...
\begin{thmframe}{Equivalenza tra le due forme di teorema di completezza}{}
    Si ha che \( T \vDash A \iff T \vdash A \ \equiv \ T \text{ coerente } \iff T \text{ ha un modello } (\in \texttt{SAT})\)
\end{thmframe}

\begin{proofframe}
    Sappiamo che \( T \vDash A \). Ci sono due opzioni:
    \begin{itemize}
        \item \( T \in \texttt{UNSAT} \) - \( T\vDash A \) è vera \( \forall A \), quindi \( T \) incoerente ()
    \end{itemize}
\end{proofframe}
