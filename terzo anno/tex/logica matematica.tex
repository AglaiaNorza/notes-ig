...
Gli enunciati che sono veri in qualunque gruppo formano l'insieme delle conseguenze logiche della teoria \(T_{\text{Gruppi}}\); ossia \(\{E : T_{\text{Gruppi}} \models E\}\). Le proprietà di un singolo gruppo \( G \) formano la ``teoria di \( G \)'' \(\{E : \mathcal{G} \models E\}\).

Si osserva facilmente che la teoria di una singola struttura \(\mathcal{M}\) è sempre completa. D'altra parte, l'insieme delle conseguenze logiche di una teoria $T$ non è necessariamente completo (nel senso di contenere \(E$ o $\neg E\) per ogni possibile enunciato $E$). Per esempio, è facile vedere che le conseguenze degli assiomi di gruppo (ossia la teoria $T_{\text{Gruppi}}$) non è un insieme completo: dato che esistono gruppi abeliani e gruppi non-abeliani e dato che la proprietà di essere abeliano è agevolmente esprimibile nel linguaggio dei gruppi con un enunciato predicativo ($C = \forall v_1 \forall v_2 (v_1 \circ v_2 = v_2 \circ v_1)$), esistono modelli di $T_{\text{Gruppi}}$ che soddisfano $C$ e modelli di $T_{\text{Gruppi}}$ che non soddisfano $C$ (ossia soddisfano $\neg C$). Dunque né $C$ né $\neg C$ sono in $\text{Conseq}(T_{\text{Gruppi}})$ (né l'essere abeliano né l'essere non abeliano sono conseguenze degli assiomi di gruppo). La teoria $T_{\text{Gruppi}}$ è dunque incompleta.
\end{gframe}

\section{La teoria DLO}

\subsection{Numeri razionali}
Prendiamo la struttura \( (\Q, \leq ) \). Le proprietà di questa struttura si possono esprimere in enunciati predicativi nel linguaggio \( \Ll = \{\leq (x,y)\}\), per il quale, per comodità, usiamo la notazione infissa \( x \leq y \). Aggiungiamo \( x < y \) come abbreviazione di \( (x \leq y) \land \neg(x = y)\). 
...
Quindi, è corretto dire che ogni struttura \( B, \leq_B \) DLO sia isomorfa a \( \Q, \leq  \)?
\end{document}
