...
Prendiamo la struttura \( \Qq = (\Q, \leq ) \). Le proprietà di questa struttura si possono esprimere in enunciati predicativi nel linguaggio \( \Ll = \{\leq (x,y)\}\), per il quale, per comodità, usiamo la notazione infissa \( x \leq y \). Aggiungiamo \( x < y \) come abbreviazione di \( (x \leq y) \land \neg(x = y)\). 
...
Vogliamo dimostrare che \textit{ogni struttura} dove \( B \) è numerabile e \( \preceq \) un ordine totale denso senza estremi su \( A \) è isomorfa a \( \Q, \leq  \).
                ...
                \item Caso \textbf{pari}: scegliamo un elemento \(p_{n+1}\) in \(B \setminus \{p_0, \dots, p_n\}\) con indice minimo in \(\{b_0, b_1, b_2, \dots\}\). Compariamo questo elemento agli elementi già scelti. 
                    ...
                    \begin{itemize}
                        \item {\color{gray}(Prima di tutti gli altri elementi)}
                            ...
                            Per ogni \(m \le n\), \(p_{n+1} < p_m\). In questo caso scelgo un \(q_{n+1}\) in \( \Q\) tale che per ogni \(m \le n\) abbiamo \(d_{n+1} < d_m\). Questo elemento esiste perché \( \Q \) soddisfa gli assiomi che asseriscono la non esistenza di estremi nell'ordine. 
                        ...
                        \item {\color{gray}(Dopo tutti gli altri elementi)}
                            ...
                            Per ogni \(m \le n\), \(d_{n+1} > d_m\). In questo caso scelgo un \(q_{n+1}\) in \(\Q \) tale che per ogni \(m \le n\) valga \(d_{n+1} > d_m\). Questo elemento esiste perché \( \Q \) soddisfa l'assioma che esclude l'esistenza di un estremo destro nell'ordine.
                            
                            {\color{gray}(Scelgo un elemento da \( \Q \) che sia anch'esso dopo tutti gli altri)}

                        \item {\color{gray}(Da qualche parte nel mezzo)}

                            Nessuno dei primi due casi. Allora esistono \(m_0, m_1 \le n\) tali che \(p_{m_0} < p_{n+1} < p_{m_1}\) e nessun altro elemento di \(\{p_0, p_1, \dots, p_n\}\) è nell'intervallo \([p_{m_0}, d_{m_1}]\). In questo caso scelgo un elemento \(d_{n+1}\) in \(\Q\) tale che \(d_{m_0} < d_{n+1} < d_{m_1}\). Questo elemento esiste perché \(\Q\) soddisfa l'assioma di densità.
...
\\\( \exists r \in \R\) \ t.c. \ \( \Rr \vDash F(x_1, x_2, y) \tworowmatrix{x_1 & x_2 & y}{q_1 & q_2 & r} \). Ne scegliamo uno in modo canonico {\color{gray}(assioma della scelta)}.

\begin{defframe}{Funzione di Skolem}{}
    Chiamiamo \textbf{funzione di Skolem} (da Thoralf Skolem) della formula \( F \) relativamente ad \( y \) la funzione 
    \[ f_{F, y} : \Qq^n \to \Rr \]
...
\( \Aa = ( A =\bigcup_{i\in \N} A_i, \leq_A) \) ha la proprietà del testimone relativamente ad \( \Rr \).
\todo{fine}
...
\begin{itemize}
    ...
    \item  $R \subseteq \mathbb{N}^k$ si dice calcolabile se la sua \textbf{funzione caratteristica} $\varphi_R$ è calcolabile.
    \subitem 
    \(
    \varphi_R(\vec{x}) = \begin{cases} 
    1 & \text{se } \vec{x} \in R \\ 
    0 & \text{se } \vec{x} \notin R 
    \end{cases}
    \)
...
Tutte le funzioni appartenenti alla classe $\mathcal{C}$ sono rappresentabili in $\mathcal{N}$ (il modello standard dell'aritmetica).

\begin{itemize}
    \item Data una funzione $\varphi: \mathbb{N}^k \rightarrow \mathbb{N}$ con $\varphi \in \mathcal{C}$.
    \item esiste una formula $F(x_1, \dots, x_k, y)$ nel linguaggio dell'aritmetica tale che:
    \item per ogni tupla di numeri naturali $(a_1, \dots, a_k, b) \subseteq \mathbb{N}^{k+1}$, vale l'equivalenza:
    \[
    \varphi(a_1, \dots, a_k) = b \iff \mathcal{N} \models F(x_1, \dots, x_k, y)[a_1, \dots, a_k, b]
    \]
\end{itemize}
