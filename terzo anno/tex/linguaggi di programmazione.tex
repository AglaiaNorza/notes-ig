...
La sintassi dell'istanziazione è \( M A \) (dove \( M \) è il termine polimorfo e \( A \) è il tipo specifico).
...
\begin{itemize}
    \item ``top-down''
        \subitem la regola dice che se abbiamo un termine \( M \) che funziona con un tipo generico \( A \) (senza vincoli esterni su \( X \)), possiamo ``impacchettarlo'' con \( \Lambda X \); il risultato è un nuovo termine polimorfo che non dipende più da un tipo fisso, ma accetta il tipo come parametro

\subitem{\color{CadetBlue}(dalla funzione specifica \( M \) creo la funzione generica \(\Lambda X. M \))}
...
\[ U = \forall X. X \to X \]
Nel Sistema \( F2 \), non possiamo applicare direttamente un termine polimorfo a un dato. Dobbiamo prima specializzarlo. Il termine che andremo a derivare è quindi:
...
\begin{itemize}
    ...
    \item \textit{Non valido:} Se $\sigma = \forall X. Y \to X$, allora \ $\forall Y. (int \to Y) \to X$ \textit{non} è istanza generica perché $Y$ è libera in $\sigma$ e non può essere catturata dalla quantificazione.
        ...
        \begin{gframe}{cattura di variabile}
            \vspace{-1em}
    \[ \sigma = \forall X.\ (Y \to X) \quad \xrightarrow{??} \quad \sigma' = \forall Y.\ ((int \to Y) \to \dots) \]
    ...
    Nello schema proposto \( \sigma' \), abbiamo aggiunto un quantificatore \( \forall Y \) - diciamo quindi ``per qualsiasi tipo \( Y \)...''
