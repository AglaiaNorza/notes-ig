...
Supponiamo di avere $x:X$ nel contesto (quindi $X$ è libero)\\
Se potessimo generalizzare:
\[ x:X \vdash \Lambda X.x : \forall X.X \]
Applicando questo termine a $int$:
\[ x:X \vdash (\Lambda X.x) \ int : int \]
...
\begin{center}
 ...
 Se consideriamo la valutazione del termine $M\ (int \to int)(M\ int)$:
 \vspace{-1.5em}
    \begin{center}
    \begin{prooftree}
