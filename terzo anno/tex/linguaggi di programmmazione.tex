...
\item \textit{livello logico: incoerenza del sistema}
    ...
    Costruiamo il termine \( M = \Lambda X.\lambda x : X.(\Lambda X.x)\ int \).
\begin{center}
    \begin{prooftree}
        \AxiomC{ $x:X \vdash x:X$}
        \UnaryInfC{$x:X \vdash \Lambda X.x : \forall X.X$}
        \UnaryInfC{$x:X \vdash (\Lambda X.x)\ int : int$}
        \UnaryInfC{$\varnothing \vdash \lambda x : X.(\Lambda X.x)\ int : X \to int$}
        \UnaryInfC{$\varnothing \vdash \Lambda X.\lambda x : X.(\Lambda X.x)\ int : \forall X.(X \to int)$}
    \end{prooftree}
    \end{center}
    ...
    Otteniamo un termine che il sistema classifica come intero, nonostante al suo interno non compaia mai nessuna costante numerica. 
