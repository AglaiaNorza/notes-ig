...
\textit{Caso induttivo (true):} se la guardia è vera: si esegue il corpo $p$ che produce uno stato intermedio $S'$, e si ri-esegue l'intero ciclo partendo da $S'$. Il risultato finale $S''$ è il risultato del ciclo.
    ...
    \item si estende l'ambiente ($E[x \mapsto l]$) e si aggiorna lo store ($S[l \mapsto v]$) (inizializzazione).
...
\item \textbf{Dereferenziazione ([ref])}:
    ...
    \end{prooftree}
        ...
        La regola converte una L-Exp in un valore leggendo dallo store.
...
\item \textbf{Assegnamento ([assign])}:
    ...
    \end{prooftree}
        ...
        Questa regola formalizza l'effetto collaterale (side-effect) dell'aggiornamento della memoria.

        {\color{gray}La regola distingue tra il calcolo del \textit{R-value} (l'espressione $M$ valutata a $v$) e del \textit{L-value} (l'espressione assegnabile $V$ valutata alla locazione $l$). Lo store $S$ viene aggiornato scrivendo $v$ nella locazione $l$.}
...
\end{center}
    ...
    \begin{enumerate}
        ...
        \item la memoria viene inizializzata con i valori $v_i$.
    \end{enumerate}
    ...
    \end{prooftree}
\end{itemize}
La regola crea una chiusura, ovvero una tripla $\langle x, p, E \rangle$.
    Viene catturato l'ambiente $E$ esistente al momento della definizione (``cattura del contesto''), quindi quando la procedura verrà chiamata, userà questo ambiente salvato (esteso con il parametro) e non quello del chiamante, garantendo così lo scoping statico.
...
\begin{cdefframe}{DeepGreenLight}{DeepGreen}{Regola Call-by-Reference}{}
    ...
    \end{prooftree}
    \vspace{-0.5em}
    {\small (*) dove $E(y) = \langle x, p, E' \rangle$.}
\end{cdefframe}
...
\begin{cdefframe}{DeepGreenLight}{DeepGreen}{Regole Call-by-Name}{}
    \begin{prooftree}
        ...
        \RightLabel{($\text{se } E(y) = \langle x, p, E' \rangle$)}
        \UnaryInfC{$E \vdash \text{call } y(V), S \stackrel{p}{\leadsto} S'$}
    \end{prooftree}
    
    \vspace{1em}
    \textbf{Nuova regola di accesso [loc3]:}
    Sostituisce [loc1] per le variabili passate per nome.
    \begin{prooftree}
        \AxiomC{$E' \vdash V, S \stackrel{l}{\leadsto} l$}
        \RightLabel{($\text{se } E(x) = (V, E')$)}
        \UnaryInfC{$E \vdash x, S \stackrel{l}{\leadsto} l$}
    \end{prooftree}
...
\section{\( Fun_{\tau}\): il sistema dei tipi di Hindley-Milner}
Il sistema dei tipi di Hindley-Milner (sistema HM) è il sisistema utilizzato dal linguaggio Standard ML. Si può formalizzare tramite 

Per garantire la decidibilità dell'inferenza dei tipi (Algoritmo W), il linguaggio ML impone delle restrizioni rispetto a F2.

\subsection{Gerarchia dei Tipi in ML}
I tipi sono divisi in due categorie:
\begin{enumerate}
    \item \textbf{Prime Types ($\tau$):} Tipi monomorfi (base o funzioni di tipi base). Non contengono $\forall$.
    \[ \tau ::= K \mid X \mid \tau \to \tau \]
    \item \textbf{Type Schemes ($\sigma$):} Schemi polimorfi dove i quantificatori sono solo esterni.
    \[ \sigma ::= \tau \mid \forall X . \sigma \]
\end{enumerate}

\begin{gframe}{Limitazione di ML}{}
    In ML, una funzione non può prendere in input un oggetto polimorfo.
    Non posso scrivere un tipo come $(\forall X. X \to X) \to int$. Il polimorfismo è "prenex": i quantificatori stanno solo in cima.
\end{gframe}

\subsection{Il Polimorfismo del Let (Let-Polymorphism)}
Dato che non possiamo passare polimorfismo tramite $fn$, ML usa il costrutto $let$ per introdurre generalizzazione.

\begin{gframe}{Sintassi e Semantica}{}
    \[ let \ x = M \ in \ N \]
    Semanticamente equivale a $(fn \ x \Rightarrow N) M$, ma viene tipato diversamente.
\end{gframe}

\subsubsection{Differenza tra $fn$ e $let$}
Consideriamo il termine auto-applicativo:
\[ fn \ x \Rightarrow x \ x \]
\begin{itemize}
    \item \textbf{In ML (con $fn$):} Non è tipabile. $x$ dovrebbe avere un tipo polimorfo per essere usata sia come funzione che come argomento, ma gli argomenti di $fn$ devono essere monomorfi ($\tau$).
    \item \textbf{In ML (con $let$):}
    \[ let \ x = (fn \ y \Rightarrow y) \ in \ (x \ x) \]
    È tipabile e corretto.
\end{itemize}

\textbf{Perché funziona?}
Nel $let$, il sistema calcola il tipo di $M$ ($fn \ y \Rightarrow y : \alpha \to \alpha$), lo \textbf{generalizza} in uno schema ($\forall \alpha. \alpha \to \alpha$) e lo assegna a $x$.
Nel corpo $N$, ogni occorrenza di $x$ può essere \textbf{istanziata} con tipi diversi (es. la prima come $(int \to int) \to (int \to int)$ e la seconda come $int \to int$).
