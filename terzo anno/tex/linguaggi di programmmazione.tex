...
(essenzialmente
\( X \in \text{free}(\Gamma)\) se non compare in \( \Gamma \) oppure se compare, ma non ``legato'' 
             (es. in \( x:X \) è libero, in  \( y: \forall X.X\to X \) è ``legato''))
...
\begin{gframe}{Condizione laterale}
    La regola di introduzione del $\forall$ impone che la variabile di tipo $X$ non sia libera nel contesto $\Gamma$ ($X \notin \text{free}(\Gamma)$).

Se violassimo questa condizione, il sistema diventerebbe logicamente incoerente.

    Supponiamo di avere $x:X$ nel contesto (quindi $X$ libero). Se si ignorasse la regola della condizione laterale, si potrebbe generalizzare $X$ anche se è già in uso.

        Questo ci permetterebbe di costruire un termine che converte ``magicamente'' una variabile $x$ (di tipo generico $X$) in un intero, forzandone il tipo:
        \vspace{-1em}
\begin{center}
    \begin{prooftree}
    ...
    \end{center}
     ...
     Chiamiamo \( M \) il termine risultante dalla derivazione sopra:
    \[ M = \Lambda X.\lambda x : X.(\Lambda X.x)\ int \]

        Usiamo \( M \) per costruire un intero dal nulla.    
    
        Considerando la valutazione del termine $M\ (int \to int)(M\ int)$:
 \vspace{-1.5em}
    \begin{center}
    \begin{prooftree}
    ...
    \end{center}
     ...
     il termine $M\ (int \to int)(M\ int)$ corrisponderebbe ad un intero, nonostante al suo interno non sia presente alcun numero intero.

Avremmo "costruito un intero fatto di aria fritta": il sistema prevederebbe l'esistenza di un intero senza sapere quale esso sia, violando la natura costruttiva del calcolo.
